    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div v-else>
            ...
            <div v-else class="space-y-4">
                <div v-for="order in orders" :key="order.Id" class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="font-bold text-gray-900">Zamówienie #{{ order.Id }}</h3>
                                <p class="text-sm text-gray-600">{{ formatOrderDate(order.OrderDate) }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="font-bold text-gray-900">{{ order.TotalAmount.toFixed(2) }} zł</p>
                            <p class="text-sm text-gray-500">{{ order.orderItems?.length || 0 }} produkt(y)</p>
                        </div>
                    </div>
...
async function loadOrders() {
    ...
    try {
        // Używamy funkcji getMyOrders z naszego kompletnego serwisu
...
function formatOrderDate(dateString) {
    return new Date(dateString).toLocaleDateString('pl-PL', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })
}
