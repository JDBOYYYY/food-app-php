                ...
                <div class="flex items-center gap-6">
                    <div class="relative">
                        <div class="h-24 w-24 bg-orange-500 rounded-full flex items-center justify-center">
                            ...
                            <img v-else :src="user.avatar" :alt="user.name" class="h-24 w-24 rounded-full object-cover" />
                        </div>
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500">Użytkownik od {{ user.memberSince }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <nav class="space-y-2">
                            <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="['w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors', activeTab === tab.id ? 'bg-orange-50 text-orange-600 border border-orange-200' : 'text-gray-700 hover:bg-gray-50']">
...
const user = computed(() => {
    if (!authStore.user) return { name: 'Ładowanie...', email: '...', created_at: new Date().toISOString() }
    return {
        ...authStore.user,
        avatar: null,
        memberSince: new Date(authStore.user.created_at).toLocaleDateString('pl-PL', { year: 'numeric', month: 'long' }),
    }
})
</script>
